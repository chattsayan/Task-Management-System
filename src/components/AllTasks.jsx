...
const AllTasks = () => {
  ...
  const applyFilters = () => {
    setToggle(false); // Closes the panel after filtering
  };
      ...
      <div className="mt-10">
        ...
        >
          ...
          <div className="flex gap-2 flex-col sm:flex-row items-center">
            <p className="font-bold text-xl text-indigo-500">Sort </p>
            ...
            <button
              ...
              className="bg-indigo-500 hover:bg-indigo-700 transition duration-300 text-white p-2 rounded-xl"
            >
              Apply Filters
            </button>
          </div>
      ...
      {filteredTasks.length > 0 ? (
        <div className="flex flex-wrap gap-y-4 gap-x-14 justify-center  overflow-auto mt-5 h-[80vh] sm:h-[80vh]">
          {filteredTasks.map((task) => (
            <TaskCard key={task.id} id={String(task.id)} {...task} />
          ))}
        </div>
      ) : (
